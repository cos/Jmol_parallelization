/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2011-06-16 09:53:22 -0500 (Thu, 16 Jun 2011) $
 * $Revision: 15599 $
 *
...
class RepaintManager {
  ...
  void pushHoldRepaint() {
    ++holdRepaint;
    // System.out.println("repaintManager pushHoldRepaint holdRepaint=" + holdRepaint + " thread=" +
  ...
  void popHoldRepaint(boolean andRepaint) {
    --holdRepaint;
    // System.out.println("repaintManager popHoldRepaint holdRepaint=" + holdRepaint + " thread=" +
    // Thread.currentThread().getName());
    if (holdRepaint <= 0) {
      holdRepaint = 0;
      if (andRepaint) {
  ...
  synchronized void requestRepaintAndWait() {
    ...
    viewer.repaint();
    try {
      // System.out.println("repaintManager requestRepaintAndWait I am waiting for a repaint: thread=" +
      // Thread.currentThread().getName());
      ...
      // System.out.println("repaintManager requestRepaintAndWait interrupted thread=" +
      // Thread.currentThread().getName());
    }
    // System.out.println("repaintManager requestRepaintAndWait I am no longer waiting for a repaint: thread=" +
    // Thread.currentThread().getName());
  }
  ...
  private ShapeRenderer allocateRenderer(int shapeID, Graphics3D g3d) {
    ...
    if (className.equals("SticksRenderer"))
      return new SticksRenderer();
